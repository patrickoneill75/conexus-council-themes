#!/usr/bin/env python3
"""Analyze one quarter's raw survey: extract feedback items, append them to the
tracker, re-upload it, then synthesize and publish this quarter's themes.

Triggered from the control panel's Analyze button — see analyze.yml for the
workflow_dispatch inputs this reads from the environment.
"""
...
from themes import box_store, claude_client, config, publish, survey_extract  # noqa: E402
...
def main() -> int:
    ...
    print(f"Downloading survey {args.survey_file_id} and tracker {tracker_file_id}...")
    survey_bytes = box_store.download(args.survey_file_id)
    tracker_bytes = box_store.download(tracker_file_id)

    t = Tracker(tracker_bytes)
    sid = make_survey_id(args.year, args.quarter, args.region)
    ...
    rows = survey_extract.response_rows(survey_bytes)
    ...
    t.append_items(items, args.year, args.quarter, args.region, source_name)
    ...
    prev_pool = rows_in(all_rows, quarters_before(args.year, args.quarter, 1, 1))
    year_pool = rows_in(all_rows, quarters_before(args.year, args.quarter, 1, 4))

...
### Step 2: the Manufacturing Workforce Needs Assessment

An assessment can be one of two kinds, chosen when the admin creates it. A **chat**
assessment is the yes/no conversation above. A **workforce** assessment is a form and a
computed picture, on its own page, with no model call anywhere in it — every number is
arithmetic over what the employer typed.

The employer adds as many roles as they have, each with four sections: the role name;
current employment (headcount, average years of experience, skills gaps); retirement risk
(how many can retire within five years, and what knowledge goes with them); and current
vacancies (vacancies now, anticipated new roles in three years, hiring difficulty).
Everything saves as they go, so they can add roles over several sittings. A button says
they have entered them all.

Then the picture:

| | |
|---|---|
| **Current workforce** | Headcount and average years of experience, weighted by headcount — a 40-person role at 12 years and a 2-person role at 1 do not average to 6.5. The three skills gaps named across the most roles, with each role expandable to see its own. |
| **Retirement risk** | Total eligible within five years, the same within three, the distribution by role, and the five pieces of knowledge named against the most roles. |
| **Employment gap** | Vacancies today; the three-year gap (vacancies plus three fifths of the retirement risk, **rounded per role** so the by-role table adds up to the headline); and the five-year gap (vacancies plus everyone eligible). Anticipated new roles are reported beside these rather than folded in, because they were not in that formula. |

**Then apprentices.** Guidance on the page: most first-year employers succeed with 1–3
apprentices per site, roughly one active mentor per 1–2 apprentices. The employer enters
how many they could take on per role per year over five years and updates the picture.

The one assumption, stated on the page: **an apprentice trains for two years and counts as
full staff from the year after** — start one in year 1 and they are staffing the role in
year 3. Apprentices still in training are shown beside the gap and never inside the filled
figure; someone two years off qualifying is not filling a vacancy today. More qualified
apprentices than vacancies reads as 100% covered, not 140%, because the surplus is growth
rather than coverage.

**Scoring.** There is none — this is planning, not a test. A finished workforce step
reports 100%, which is what lets an admin put a threshold on it and have "finished" mean
"passed". The dashboard shows it as **Complete**.

**Box.** Optional, per assessment. On finish, one CSV per respondent — every role, its
gap on all three horizons, and the apprentice plan — upserted by name into the folder the
admin picked. The authoritative copy is always KV; Box is the export, and a failure there
...
account per assessment holding which items are ticked, plus one `workforce:` entry per
account per workforce assessment holding their roles and apprentice plan. Unlike Consensus, the run state (the cursor, the scores, the
